...
RUN conda create -n cache Cython jinja2 scipy matplotlib requests scikit-image pytest pyyaml pandas nomkl pytest-cov coverage sphinx
# Create the same cache with conda-forge packages
RUN conda create -c conda-forge -n cache2 Cython jinja2 scipy matplotlib requests scikit-image pytest pyyaml pandas nomkl pytest-cov coverage sphinx
